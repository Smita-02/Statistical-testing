...
def build_not_applicable_fairness_report(df, sensitive_column, reason):
    sensitive_features = prepare_sensitive_feature(df, sensitive_column)
    ...
    return {
        ...
        "distribution": sensitive_feature_distribution(sensitive_features),
        "label_mapping": {},
        "fairness_status": "NOT_APPLICABLE",
        "fairness_unavailable_reason": reason,
        "observed_labels": [],
        "demographic_parity": {
            "demographic_parity_difference": None,
...
def evaluate_fairness_thresholds(fairness_metrics):
    ...
    disparate_impact_ratio = fairness_metrics.get(
        "disparate_impact_ratio"
    )

    demographic_parity_difference = fairness_metrics.get(
        "demographic_parity_difference"
    )

    if (
        disparate_impact_ratio is None
        or demographic_parity_difference is None
    ):

        return {
            "overall_fairness_status": "NOT_EVALUATED",
            "violations": [
                "Fairness thresholds could not be evaluated because one or more fairness metrics are unavailable"
            ],
        }
    ...
    if abs(demographic_parity_difference) > 0.10:
        status["overall_fairness_status"] = "FAILED"
...
def run_fairness_analysis(df, y_true, y_pred, sensitive_column, task_type="binary_classification"):
    """
    ...
    if task_type != "binary_classification":
        return build_not_applicable_fairness_report(
            df=df,
            sensitive_column=sensitive_column,
            reason=f"DIR and DPD are currently supported only for binary classification models, not '{task_type}'",
        )
...
def run_multi_fairness_analysis(df, y_true, y_pred, sensitive_columns, task_type="binary_classification"):
    """
